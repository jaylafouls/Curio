...
> **Changelog v5.7 (recette Landing, 2026-08)** : ajout §21 — barre de fidélité visuelle V1 assouplie de "pixel-perfect vs. maquette" à "propre et pro" (CEO/PO). La maquette devient une référence directionnelle, pas une spec littérale ; liberté de changer police/placements/terminologie et de retirer des éléments (ex : motif orbital du hero) sans nouvel arbitrage à chaque fois. Ne touche pas §0 (spec > maquettes) sur le plan fonctionnel, uniquement le rendu visuel.
...
15. **[Phase 5, nouveau]** « Inspiration sans URL » (cf. §20) — sauvegarder une inspiration visuelle sans lien source. En tension avec « 1 URL = 1 objet » (`links.url_normalized NOT NULL UNIQUE`, `user_links.url_origin NOT NULL`). Supposerait un second type d'objet ou une URL synthétique — décision de modèle de données. **Différé, à trancher séparément** ; l'ancien point d'entrée « Add Custom Image » a été retiré en attendant (redondant avec l'upload in-flow).
---
## 20. Point d'entrée « Add Custom Image » retiré — piste « inspiration sans URL » différée
**[Phase 5, chantier Add-to-Curio + Saved]**
**Décision (actée CEO/PO)** : le point d'entrée dédié **« Add a custom image »** du panneau « Add to Curio » (§2.1/§3.1 du Add Item Flow) est **supprimé**. Il était redondant avec l'ajout d'image déjà disponible **à l'intérieur** du Save Flow (étape Customize) : la même capacité d'upload vers le bucket `user-images` reste offerte à tout save, sans un second point d'entrée qui dupliquait la fonction.
**Ce qui change concrètement** :
- Panneau « Add to Curio » : deux actions au lieu de trois (Save link + Create collection).
- `SaveFlowModal` : suppression de la prop `initialAction` et de son type `SaveFlowInitialAction` (`'save' | 'image'`), du ref/effect `autoPickImage` qui ouvrait automatiquement le sélecteur de fichier. L'upload in-flow (bouton « Add a custom image » de l'étape Customize) est **conservé tel quel**.
- Clés i18n orphelines retirées (`SaveFlow.actionAddImage`, `SaveFlow.actionAddImageHint`) EN/FR.
**Piste future séparée — « inspiration sans URL » (NON construite)** : l'alternative qui aurait justifié un point d'entrée image autonome — permettre de sauvegarder une **inspiration visuelle sans URL** — est **mise de côté comme piste distincte**, pas abandonnée silencieusement. Elle est en **tension directe avec le principe canonique du modèle de données « 1 URL = 1 objet »** : `links.url_normalized text NOT NULL UNIQUE` + `user_links.url_origin text NOT NULL` (Data Model §8/§9, migration `0002`). Un Item sans URL casserait la clé de canonicalisation et la déduplication cross-user (le cœur du modèle : un Link partagé, des `user_links` personnels). La construire supposerait un second type d'objet (ou une URL synthétique), décision de modèle de données à trancher séparément — **hors périmètre de ce chantier**.
**À trancher (nouveau point ouvert)** : voir §11 point 15.

---

## 21. Barre de fidélité visuelle V1 — assouplie de "pixel-perfect" à "propre et pro" (recette Landing, 2026-08)

**Contexte** : plusieurs allers-retours de recette sur la Landing (hero orbital, header/photo, alignements) visaient une fidélité pixel-perfect à `01-curio-reference-4x.png`. Le CEO/PO a constaté que cette barre coûtait trop de cycles pour trop peu de valeur perçue, créait une impression de surplace, et bloquait l'avancement page par page attendu avant l'ouverture. Décision explicite de rouvrir des points jusqu'ici traités comme acquis (police, placements, motif orbital, terminologie), y compris certains déjà consignés dans ce journal.

**Décision actée (CEO/PO)** :

**21.1 — La maquette designer devient une référence directionnelle, pas une spec littérale**, pour la Landing et les pages suivantes traitées en V1. Elle continue de guider l'ambiance, la palette, la densité et l'esprit général ; elle cesse d'imposer une conformité au pixel (coordonnées exactes, polices, ombres, micro-espacements). Le critère d'acceptation devient : propre, cohérent avec la charte, sans bug visuel — pas "identique à la maquette".

**21.2 — Liberté de modification étendue, y compris sur des points déjà tranchés.** Le CEO/PO peut changer la police globale, déplacer/simplifier des blocs, retirer des éléments jugés trop complexes ou trop fragiles à maintenir (le motif orbital "You + Topics" du hero est explicitement cité comme candidat), et reformuler la terminologie publique — sans qu'un nouvel arbitrage formel soit requis à chaque changement. Un élément retiré pour simplifier la V1 peut être réintroduit plus tard ; ce n'est pas un abandon de l'identité de marque (orbite/points comme métaphore de Curio, §1), seulement un report tactique.

**21.3 — Méthode de travail : page par page, "beau et utile, rien qui foire".** On avance page par page plutôt que de chercher l'exhaustivité sur une seule page. Priorité : que l'outil fonctionne et soit agréable à utiliser ; la beauté vient en second, en cherchant le compromis plutôt que la copie exacte du design fourni.

**Raison** : la fidélité pixel-perfect a un coût d'itération élevé et une valeur produit faible tant que l'outil de base n'est pas stabilisé pour la V1 ; le risque de blocage avant ouverture publique est jugé plus grave que l'écart avec la maquette. Le CEO/PO préfère un outil simple, fonctionnel et cohérent, quitte à raffiner l'habillage visuel plus tard une fois la base solide.

**Conséquence** :
- Les décisions §12 (Landing Light), §19 (nav, presse, lexique) restent valables sur le fond ; seule la **méthode de recette** change — un futur écart de police/placement/terminologie sur ces sujets n'est pas une régression à signaler systématiquement, sauf s'il touche à l'honnêteté du produit (cf. §19.2, principe inchangé : pas de contenu trompeur).
- Le motif orbital du hero n'est pas retiré par cette entrée — il reste en l'état actuel (version simplifiée à rayon fixe, cf. commits `recette/pre-beta-fixes`). Sa suppression éventuelle sera actée séparément si le CEO/PO la confirme.
- §0 ("la spec prime sur les maquettes") reste inchangé pour les fonctionnalités, la structure des objets et les règles métier — cet assouplissement ne concerne que le rendu visuel.