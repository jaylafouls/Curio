...
14. **[Phase 5, nouveau]** « Inspiration sans URL » (cf. §19) — sauvegarder une inspiration visuelle sans lien source. En tension avec « 1 URL = 1 objet » (`links.url_normalized NOT NULL UNIQUE`, `user_links.url_origin NOT NULL`). Supposerait un second type d'objet ou une URL synthétique — décision de modèle de données. **Différé, à trancher séparément** ; l'ancien point d'entrée « Add Custom Image » a été retiré en attendant (redondant avec l'upload in-flow).

---

## 19. Point d'entrée « Add Custom Image » retiré — piste « inspiration sans URL » différée
...
**À trancher (nouveau point ouvert)** : voir §11 point 14.
